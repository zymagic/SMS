...
fun SMSContact.display() : Pair<String, String> {
    ...
    if (stored == null) {
        val cursor = App.app.contentResolver.query(Uri.withAppendedPath(ContactsContract.Contacts.CONTENT_LOOKUP_URI, Uri.encode(person.toString())),
...
private fun SMSContact.displayDefault(): Pair<String, String> {
    val d = displayName
    val f = fullName
    if (d != null && f != null) {
        return d to f
...
private fun String.thumb(): String {
    val c = get(0).toInt()
    if ((c and 0xff) != 0 && ((c ushr 4) and 0xff) != 0) {
        return c.toString()
    } else {
        return substring(0, min(2, length))
    }
}